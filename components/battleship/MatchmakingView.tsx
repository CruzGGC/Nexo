        ...
        <div className="grid gap-4 w-full max-w-md">
          ...
          >
            ...
            <div>
              ...
              <p className="text-sm text-slate-500 dark:text-slate-400">Tens um código de convite? Entra aqui.</p>
            </div>
          </button>
          
          <button
            onClick={onCancel}
            className="mt-4 text-sm text-slate-500 hover:text-slate-800 dark:hover:text-slate-200"
          >
            Voltar
          </button>
        </div>
      ...
      {view === 'create' && (
        <div className="w-full max-w-md space-y-6 rounded-2xl border border-slate-200 bg-white p-8 dark:border-slate-800 dark:bg-slate-900">
          <div className="text-center">
            <h3 className="text-xl font-bold text-slate-900 dark:text-white">A tua Sala Privada</h3>
            <p className="mt-2 text-sm text-slate-500">Partilha este código com o teu amigo.</p>
          </div>

          <div className="flex items-center justify-center gap-4 rounded-xl bg-slate-100 p-4 dark:bg-slate-800">
            <code className="text-3xl font-mono font-bold tracking-widest text-indigo-600 dark:text-indigo-400">
              {generatedCode}
            </code>
            <button
          ...
          <div className="flex gap-3">
            <button
              ...
              className="flex-1 rounded-xl border border-slate-200 py-3 font-semibold text-slate-600 hover:bg-slate-50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-800"
            >
              Voltar
            </button>
            <button
      ...
      {view === 'join' && (
        <div className="w-full max-w-md space-y-6 rounded-2xl border border-slate-200 bg-white p-8 dark:border-slate-800 dark:bg-slate-900">
          <div className="text-center">
            ...
            <p className="mt-2 text-sm text-slate-500">Insere o código que recebeste.</p>
          </div>
          ...
          <input
            ...
            placeholder="EX: A1B2"
            className="w-full rounded-xl border-2 border-slate-200 bg-slate-50 p-4 text-center text-2xl font-bold tracking-widest outline-none focus:border-emerald-500 focus:bg-white dark:border-slate-700 dark:bg-slate-800 dark:focus:border-emerald-500"
            maxLength={4}
          />
